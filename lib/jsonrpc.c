 ...
 *
 *   - 0: No error occured but the message is too large to be received in one
 *        run. The method can be called again to continue receiving.
 *
...
jsonrpc_recv_block(struct jsonrpc *rpc, struct jsonrpc_msg **msgp)
{
    for (;;) {
        ...
        if ((error && error != EAGAIN) || (!error && *msgp)) {
            fatal_signal_run();
            return error;
        }
