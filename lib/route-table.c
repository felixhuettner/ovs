                        ...
                        uint32_t ifindex)
{
    ...
    struct route_data_nexthop *nh = &change->rd.nexthops[change->rd.n_nexthops];
                            ...
                            bool ipv4)
{
    ...
    while (len >= sizeof(*rtnh) && len >= rtnh->rtnh_len &&
        ...
        struct ofpbuf buf;
        ofpbuf_use_const(&buf, RTNH_DATA(rtnh), rtnh->rtnh_len - sizeof(*rtnh));
        if (ipv4) {
...
route_table_parse(struct ofpbuf *buf, void *change_)
{
    ...
    if (parsed) {
        ...
        if (attrs[RTA_GATEWAY]) {
            ...
            if (!route_table_parse_multipath(change, attrs[RTA_MULTIPATH], ipv4)) {
                return 0;
            }
        }
    } else {
